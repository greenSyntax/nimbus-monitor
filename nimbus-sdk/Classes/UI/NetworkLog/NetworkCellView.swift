...
public class NetworkCellView: UITableViewCell {
    
    static let identifier = "NetworkCellView"
    ...
    func setData(_ data: NetworkCellData) {
        self.labelTagTitle.text = data.statusCode
        self.viewTag.backgroundColor = data.tagColor
        self.labelTitle.text = "\(data.httpVerb.uppercased()) \(data.endpoint)"
        self.labelDescription.text = "\(data.baseURL) \n \(data.queryParams)"
        self.labelFooter.text = data.timestamp
    }
